...
# If an operator supplied INFLUX_TOKEN via environment, preserve it and skip the
# CLI-based token creation flow. This avoids deleting a user-provided token
# (the script writes the env token to /run/influx_token early above).
if [ -n "${INFLUX_TOKEN:-}" ]; then
  echo "INFLUX_TOKEN provided via environment; skipping CLI token creation and preserving /run/influx_token"
else
